...
"""
Utility functions for Computer Exercise 1: Optimization in TSBB33 3D Computer Vision
...
def load_stereo_pair():
    ...
    pair = 'frame'
    return [
        load_image_grayscale(LAB1_IMAGE_DIRECTORY / f'{pair}{i:02d}.png') 
        for i in (1, 2)
    ]
...
def cut_out_rois(image, col_indices, row_indices, roi_size):
    """Cut out regions of interest from an image (RGB image)
    how to use this???? do you need to search over row and col?
    Parameters
    -----------------
    image : (M, N) array
        Grayscale image
    ...
    rois = [image[r-m:r+m+1, c-m:c+m+1] for r,c in zip(row_indices, col_indices)] # choose a region of interest in the image
    # how to find row_indices 
    return rois
